...
export default function store(state = defaultState, action) {
  switch (action.type) {
    ...
    case 'CMD':
      ...
      const state_ = { ...state, orders: [...ordersSlice.filter(a => !a.encaisse &&(a.status?.task_name ==="servie" || a.status?.task_name ==="en_attente" || a.status?.task_name === "valide"))] }
      return state_;
    case 'CMD_CAISSE':
    ...
    case 'NEW_ORDER':
      ...
      const state_2 = { ...state, orders: [...orders4.filter(a => !a.encaisse && (a.status?.task_name ==="servie" || a.status?.task_name ==="en_attente" || a.status?.task_name === "valide"))] }
