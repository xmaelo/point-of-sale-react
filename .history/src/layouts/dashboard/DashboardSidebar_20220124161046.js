...
export default function DashboardSidebar({ isOpenSidebar, onCloseSidebar }) {
  ...
  const admin = useSelector(p => p.role && p.role.task_name &&['admin', "caissier"].includes(p.role.task_name)) 
  ...
  if(admin){
    ...
    // if(!sidebarConfig.find(a =>a.title === "Historique")){
    //   sidebarConfig.push(
    //     {
    //       title: 'Historique',
    //       path: '/dashboard/historic',
    //       icon: <Icon icon="fa-solid:history"  width={22} height={22}/>
    //     }
    //   )
    // }
    if(!sidebarConfig.find(a =>a.title === "Historique")){
