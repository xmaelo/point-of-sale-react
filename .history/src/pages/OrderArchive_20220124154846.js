...
export default function OrderArchive() {
  ...
  async function onLoadTyeOnWait(contranit){
    try {
      ...
      setShowHeigh(0) 
      setTableLoad(true)
      const cmds = await request_get('commandes?order[id]=desc&'+contranit)
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - cmds.filter(a =>a.status.task_name !== "en_attente").length) : 0;
  ...
  function renerPrice(){
    let price = 0
    cmds&&cmds.filter(a =>a.status.task_name !== "en_attente").map(o => {
      ...
      <Container>
        ...
        <Card>
            ...
            <div style={{padding: '10px', display: 'flex'}}>
              ...
              <Table>
                <UserListHead
                  ...
                  rowCount={cmds.filter(a =>a.status.task_name !== "en_attente").length}
                  numSelected={selected.length}
                  onRequestSort={handleRequestSort}
                  onSelectAllClick={handleSelectAllClick}
                />
                <TableBody>
                  {!tableLoad && cmds.filter(a =>a.status.task_name !== "en_attente")
          ...
          <TablePagination
            ...
            component="div"
            count={cmds.filter(a =>a.status.task_name !== "en_attente").length}
