...
export default function Bilan(props) {
    ...
    const handleChange = (event) => {
        ...
        if(s === "sem"){
            ...
            // console.log('timstamp1', firstDay.toISOString())
            const startDate = new Date(event.setDate(event.getDate() - event.getDay()+ (event.getDay() == 0 ? -6:1) )).toString()
            const endDate = new Date(event.setDate(event.getDate() - (event.getDay() == 0 ? 7 : event.getDay()) + 7)).toString()
            const constraint = "time[after]="+startDate.toISOString().split('T')[0]+"&time[before]="+endDate.split('T')[0]
            return onLoadTyeOnWait(constraint)
        }
        if(s === "mois"){
