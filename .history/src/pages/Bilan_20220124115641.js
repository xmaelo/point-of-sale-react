...
export default function Bilan(props) {
    ...
    async function onLoadTyeOnWait(contranit){
        try {
          ...
          const result =  await request_get('commandes?user.username='+username+'&order[id]=desc&'+contranit)
    ...
    function format (number){
      return new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'XAF' }).format(number)
    }
            ...
            <Grid item xs={12} sm={3} md={3}>
                ...
                <Card style={{padding: '10px'}}>
                    ...
                    <span style={{fontSize: "14px", fontWeight: 'bold'}}>
                        Total:{" "} {format(renerPrice())} FCFA
                    </span>
                    <br/>
                    <span style={{fontSize: "14px", fontWeight: 'bold'}}>
                        Consommation: {" "}{format(renerPrice2())} FCFA
                    </span>
                    <br/>
                    <span style={{fontSize: "14px", fontWeight: 'bold'}}>
                        Commission: {" "}{ format((renerPrice() * commission)/100)  } FCFA
                    </span>
                    <br/>
                    <span style={{fontSize: "14px", fontWeight: 'bold'}}>
                        Commission finale: {" "}{format(((renerPrice() * commission)/100) -  renerPrice2())} FCFA
                    </span>
                </Card>
            </Grid>
        </Grid>
