    ...
    <div>
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          ...
          />
          <br/>
          Montant à rembourser: <br/> 
            {parseInt(amountReceive) > 0 ? 
               <span>
                   {format(parseInt(amountReceive))+" - "+format(order.price) }  <span style={{color: "red"}}> {format(parseInt(amountReceive) - order.price)}</span>
                </span> 
                : null
            }
        </DialogContent>
