...
export default function OrderMoreMenu({order}) {
  ...
  async function handleClose(){
    setOnEncaisse(null, ()=>console.log('setOn encaiser', encaisseObj))
  }
    ...
    <>
      ...
      >
        ...
        {role !== "serveur"&& ["valide", "servie"].includes(order?.status?.task_name) &&
          <MenuItem sx={{ color: 'text.secondary' }} onClick={()=>onEncaisse(order.id)}>
            <ListItemIcon>
            ...
            <EncaisseDialog order={order} show={encaisseObj ?encaisseObj[order.id] : false} handleClose={handleClose}/>
          </MenuItem>
        }
