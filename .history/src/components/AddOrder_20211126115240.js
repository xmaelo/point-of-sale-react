...
export default function AddOrder(props) {
  ...
  async function onSaveCommande(){
    console.log('status status status Commande ', status, table)
    try {
      const obx = {
          table: table,
      ...
      console.log('obx obx obx obx', obx)
      //const result = await request_post("commandes", obx)
      const r = socket.send(JSON.stringify(obx));
      console.log('result result result', r)
      } catch (error) {
        //setLoand(false)
                                ...
                                <div>
                                    <Paper elevation={3} >
                                        <div style={{padding: '20px'}}>
                                            Date: {date}
                                            <br/>
                                            Table: {table&&tables.filter(t =>t['id'] === table)[0].name}
