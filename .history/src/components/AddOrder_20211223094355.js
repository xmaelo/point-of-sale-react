...
export default function AddOrder(props) {
    ...
    const [idTask, setTask] = React.useState({});
    ...
    async function onLoadNonFacturer(){
      try {
        const result =  await request_get('order_states?page=1&task_name=non_facturer')
        if(result&&result['hydra:member']&&result['hydra:member'].length > 0){
          setTask(result['hydra:member'][0])
        }
      } catch (error) {
        console.log('onLoadTyeOnWait', error)
      }
  ...
  async function onSaveCommande(){
    try {
      ...
      const now = new Date()
      console.log('now now now', now, now.getTime())
      
      let state = checked ? idTask : status
      const obx = {
        table: table,
                        ...
                        {activeStep===1&&
                            <div style={{paddingTop: '100px'}}>
                                <div style={{marginLeft: '25%'}}>
                                    ...
                                    <br/>
                                    <FormGroup>
                                      <FormControlLabel control={<Checkbox checked={checked} onChange={handleChangeCheck} />} label={"Ma commande "} />
                                    </FormGroup>
                                </div>
                                <div>
                                    <Paper elevation={3} >
